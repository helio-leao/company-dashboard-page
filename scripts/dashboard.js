...
function createTabulationButtonsListeners(
  buttonsClass,
  buttonActiveClass,
  buttonFunction
) {
...
function configMenuTabsNavigation() {
  ...
  createTabulationButtonsListeners(
    buttonsClass,
    buttonActiveClass,
    (button) => {
  ...
  const dropdownButtons = Array.from(
    document.querySelectorAll(".menu-button-dropdown")
  );
  ...
  dropdownButtons.forEach((button) => {
    button.addEventListener("click", () => {
      const previouslySelectedDropdown = dropdownButtons.find((aux) => {
        return (
          button !== aux &&
          aux.classList.contains("menu-button-dropdown--active")
        );
      });

      if (previouslySelectedDropdown)
        previouslySelectedDropdown.classList.remove(
...
function configInsightsTabsNavigation() {
  ...
  const tabsButtons = Array.from(
    document.querySelectorAll(".insights-table-navigation__button")
  );

  const insightsTabsButtonsPrev = document.querySelector(
    ".insights-table-navigation__button-prev"
  );
  const insightsTabsButtonsNext = document.querySelector(
    ".insights-table-navigation__button-next"
  );
...
async function loadUser() {
  ...
  if (data.organizationLogo == null) {
    organizationLogoElement = createPhotoPlaceholderStringElement(
      data.organization,
  ...
  if (data.photo == null) {
    userAvatarElement = createPhotoPlaceholderStringElement(
      data.username,
...
async function loadMenu() {
  ...
  data.menuTree.forEach((menu, index) => {
    const sideBarMenuItem = `<li class="menu__item">
      ...
      !menu.hasChildren && index === 0 ? "menu-button-final--active" : ""
    }">
                                    ${Icons[menu.name]}
                                    ${menu.name}
                                    ${
                                      menu.hasChildren
                                        ? `<svg class="menu__dropdown-icon" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                ${
                                  menu.hasChildren
                                    ? getSubmenuStringBlock(menu.name)
                                    : ""
                                }
                            </li>
    `;
    ...
    const tableRow = `<tr class="insights-table__row">
                        ...
                        <td class="insights-table-data insights-table-data--product">
                            <div class="product">
                                ...
                                }" alt="product-image" class="insights-table-data__image">
                                <span class="insights-table-data__description">${
...
async function loadResellersRanking() {
  ...
  const resellersRankingList = document.querySelector(
  ...
  data.resellers.forEach((reseller, index) => {
    ...
    if (reseller.photo == null) {
      avatarElement = createPhotoPlaceholderStringElement(
        reseller.name,
    ...
    const resellerItem = `<li class="resellers-ranking__item">
                            ...
                            <div class="resellers-ranking-data">
                                ...
                                <div class="resellers-ranking-data-sales ${
                                  isPercentageNegative
                                    ? "resellers-ranking-data-sales--negative"
                                    : ""
                                }">
...
function getNameInitials(name) {
  ...
  if (separatedNames.length > 1)
    return separatedNames[0].charAt(0) + separatedNames[1].charAt(0);
...
async function loadSales() {
  ...
  const revenueElement = document.querySelector(
    ".report-data-cards-data__value--revenue"
  );
  const totalSalesElement = document.querySelector(
    ".report-data-cards-data__value--total-sales"
  );
  const averageTicketElement = document.querySelector(
    ".report-data-cards-data__value--average-ticket"
  );
